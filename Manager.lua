...
local function getRow(i)
  ...
  row.exp = tinyBtn(row, "Export", 56); row.exp:SetPoint("RIGHT", -2, 0)
  row.del = tinyBtn(row, "Delete", 56); row.del:SetPoint("RIGHT", row.exp, "LEFT", -3, 0)
  row.ren = tinyBtn(row, "Rename", 56); row.ren:SetPoint("RIGHT", row.del, "LEFT", -3, 0)
  row.use = tinyBtn(row, "Use", 50);   row.use:SetPoint("RIGHT", row.ren, "LEFT", -3, 0)
...
function Manager.Refresh()
  ...
  for i, it in ipairs(items) do
    ...
    if it.kind == "boss" then
      ...
      row.use:Hide()
      row.ren:Hide()
      row.del:Show(); row.del:SetScript("OnClick", function() ns.DB.DeleteBoss(id); Manager.Refresh() end)
      row.exp:Show(); row.exp:SetScript("OnClick", function()
      ...
      row.use:SetScript("OnClick", function() ns.DB.SetActive(id, it.index); Manager.Refresh() end)
      row.ren:Show()
      row.ren:SetScript("OnClick", function() promptRename(id, it.index, it.p.label) end)
      row.del:Show()
      row.del:SetScript("OnClick", function() ns.DB.DeletePlan(id, it.index); Manager.Refresh() end)
      row.exp:Show()
