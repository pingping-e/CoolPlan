...
_A Mythic+ tool that auto-builds a 5-player cooldown plan from the top teams on Warcraft Logs, then plays it back in game on each boss's dangerous casts._
...
player to your real character names so reminders go to the right person, then
copy the string.
...
own cooldowns on a HUD, with a countdown and a depleting bar.

## Features

**Saved plan library.** Multiple named plans per boss, a dungeon to boss browser,
...
**Timeline preview.** Scrub the whole plan, zoom/pan, and **Test** the playback (1x, or
Shift-click for 3x) to rehearse before the pull.
...
Also a **minimap button** (collects into MinimapButtonButton / Titan / etc.) and
...
- **CurseForge:** search for **CoolPlan** (recommended, it auto-updates).
- **Manual:** download, unzip, and copy the **`CoolPlan`** folder into
  `World of Warcraft/_retail_/Interface/AddOns/`. Restart WoW.

## Notes

- No external dependencies. It bundles LibStub, LibDataBroker-1.1, LibDBIcon-1.0,
  LibDeflate and LibSharedMedia-3.0 under `Libs/` (each under its own license).
